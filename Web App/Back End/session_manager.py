    ...
    class SessionManager:
        ...
        def create_session(self, session_id: str, access_token: str, refresh_token: str, 
            ...
            try:
                ...
                # Clean up any existing sessions for the same user (if user_info provided)
                if user_info and 'username' in user_info:
                    self._cleanup_user_sessions(user_info['username'])
        ...
        def _cleanup_user_sessions(self, username: str):
            """Clean up all existing sessions for a specific user"""
            sessions_to_remove = []
            for session_id, session in self.sessions.items():
            ...
            for session_id in sessions_to_remove:
                print(f"Removing existing session for user {username}: {session_id[:10]}...")
                self.remove_session(session_id)
        ...
        def cleanup_inactive_sessions(self, max_inactive_hours: int = 24):
            """Remove sessions that haven't been accessed for a specified time"""
            cutoff_time = datetime.now() - timedelta(hours=max_inactive_hours)
            inactive_sessions = []
            
            for session_id, session in self.sessions.items():
                last_accessed = datetime.fromisoformat(session.get('last_accessed', session['created_at']))
                if last_accessed < cutoff_time:
                    inactive_sessions.append(session_id)
            
            for session_id in inactive_sessions:
                print(f"Removing inactive session: {session_id[:10]}...")
                self.remove_session(session_id)
